...
<#
    ...
    Usage :
        powershell -ExecutionPolicy Bypass -File release\instapk_mosqueadmin.ps1 <action>

    Actions :
        apk         Compile la release (gradlew :mosque-admin:assembleRelease),
                    aligne (zipalign) et signe (apksigner) avec tawkit.jks
                    -> genere release\mosque-admin.apk
        install     Installe/met a jour release\mosque-admin.apk sur l'appareil
                    adb courant
        help        Affiche ce message (comportement par defaut si aucun
                    parametre n'est fourni)

    Exemples :
        powershell -ExecutionPolicy Bypass -File release\instapk_mosqueadmin.ps1 apk
        powershell -ExecutionPolicy Bypass -File release\instapk_mosqueadmin.ps1 install
#>
...
$OutputApk   = Join-Path $ReleaseDir 'mosque-admin.apk'
$AlignedApk  = Join-Path $ReleaseDir 'mosque-admin-aligned.tmp.apk'
$GradlewBat  = Join-Path $ProjectRoot 'gradlew.bat'

$PackageName  = 'net.tawkit.mosqueadmin'
$KeyAlias     = 'tawkit'
...
function Show-Help {
    Write-Host ""
    Write-Host "instapk_mosqueadmin.ps1 - build / signature / deploiement de Mosque Admin" -ForegroundColor Yellow
    Write-Host "=========================================================================="
    Write-Host ""
    Write-Host "Usage :"
    Write-Host "  powershell -ExecutionPolicy Bypass -File release\instapk_mosqueadmin.ps1 <action>"
    Write-Host ""
    Write-Host "Actions disponibles :" -ForegroundColor Yellow
    Write-Host "  apk       Compile, aligne et signe -> release\mosque-admin.apk"
...
function Invoke-BuildApk {
    ...
    Write-Info "==> Compilation release (gradlew :mosque-admin:assembleRelease)..."
    Push-Location $ProjectRoot
    try {
        & $GradlewBat ':mosque-admin:assembleRelease'
        if ($LASTEXITCODE -ne 0) { throw "Echec de gradlew :mosque-admin:assembleRelease (code $LASTEXITCODE)" }
    } finally {
        Pop-Location
    }

    $unsignedApk = Get-ChildItem (Join-Path $ProjectRoot 'mosque-admin\build\outputs\apk\release') `
        -Filter '*-release-unsigned.apk' -File -ErrorAction SilentlyContinue |
        Select-Object -First 1 -ExpandProperty FullName
    if (-not $unsignedApk) { throw "APK release non-signe introuvable sous mosque-admin\build\outputs\apk\release" }
...
function Invoke-Install {
    if (-not (Test-Path $OutputApk)) {
        throw "release\mosque-admin.apk introuvable. Lancez d'abord : instapk_mosqueadmin.ps1 apk."
    }
    $sdkDir = Get-AndroidSdkDir
